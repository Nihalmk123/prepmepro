...
<div id="webinarModal" class="wm-overlay" aria-hidden="true">
  ...
  // Run AFTER injection — all DOM nodes guaranteed to exist now
  _setActiveNavLink();
  _bindSidebar();         // ← was IIFE before, now called here safely
}

/* ══════════════════════════════════════════
   PRIVATE — called only from injectLayout()
   ══════════════════════════════════════════ */
...
function _bindSidebar() {
  ...
  // Guard — if any element missing, bail safely
...
function openWebinarModal() {
  ...
  if (btn)         { btn.disabled = false; btn.innerHTML = "Reserve My Free Seat &rarr;"; }
...
function bindGlobalEvents() {
  // Prevent double-binding if initLayout runs more than once
...
function initLayout(currentPage) {
  injectLayout(currentPage);   // inject HTML + bind sidebar inside
  bindGlobalEvents();          // delegated events — once only
}
